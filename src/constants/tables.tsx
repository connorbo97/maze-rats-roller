declare type TableDef = {
  [group: string]: Array<string>;
};
declare type Table = {
  table: TableDef;
  label: string;
  defaultNumRolls?: number;
  valueSeparator?: string;
  maxNumRolls?: number;
};
export const TABLE_NAMES = {
  MAGIC_TABLE_ROLL: "MAGIC_TABLE_ROLL",
  MAGIC_PHYSICAL_EFFECTS: "MAGIC_PHYSICAL_EFFECTS",
  MAGIC_PHYSICAL_ELEMENTS: "MAGIC_PHYSICAL_ELEMENTS",
  MAGIC_PHYSICAL_FORMS: "MAGIC_PHYSICAL_FORMS",
  MAGIC_ETHEREAL_EFFECTS: "MAGIC_ETHEREAL_EFFECTS",
  MAGIC_ETHEREAL_ELEMENTS: "MAGIC_ETHEREAL_ELEMENTS",
  MAGIC_ETHEREAL_FORMS: "MAGIC_ETHEREAL_FORMS",
  MAGIC_MUTATIONS: "MAGIC_MUTATIONS",
  MAGIC_INSANITIES: "MAGIC_INSANITIES",
  MAGIC_CATASTROPHES: "MAGIC_CATASTROPHES",
  CHARACTER_SEX: "CHARACTER_SEX",
  CHARACTER_RACE: "CHARACTER_RACE",
  CHARACTER_APPEARANCE: "CHARACTER_APPEARANCE",
  CHARACTER_PHYSICAL_DETAIL: "CHARACTER_PHYSICAL_DETAIL",
  CHARACTER_CLOTHES: "CHARACTER_CLOTHES",
  CHARACTER_PERSONALITY: "CHARACTER_PERSONALITY",
  CHARACTER_MANNERISM: "CHARACTER_MANNERISM",

  GOTHIC_BACKGROUND: "GOTHIC_BACKGROUND",

  MONSTER_TYPE: "MONSTER_TYPE",
  MONSTER_AERIAL_FORM: "MONSTER_AERIAL_FORM",
  MONSTER_TERRESTRIAL_FORM: "MONSTER_TERRESTRIAL_FORM",
  MONSTER_AQUATIC_FORM: "MONSTER_AQUATIC_FORM",
  MONSTER_FEATURES: "MONSTER_FEATURES",
  MONSTER_TRAITS: "MONSTER_TRAITS",
  MONSTER_ABILITIES: "MONSTER_ABILITIES",
  MONSTER_TACTICS: "MONSTER_TACTICS",
  MONSTER_PERSONALITY: "MONSTER_PERSONALITY",
  MONSTER_WEAKNESS: "MONSTER_WEAKNESS",

  NPC_OCCUPATION_CIVILIZATION: "NPC_OCCUPATION_CIVILIZATION",
  NPC_OCCUPATION_UNDERWORLD: "NPC_OCCUPATION_UNDERWORLD",
  NPC_OCCUPATION_WILDERNESS: "NPC_OCCUPATION_WILDERNESS",
  NPC_ASSETS: "NPC_ASSETS",
  NPC_LIABILITIES: "NPC_LIABILITIES",
  NPC_GOALS: "NPC_GOALS",
  NPC_MISFORTUNES: "NPC_MISFORTUNES",
  NPC_MISSIONS: "NPC_MISSIONS",
  NPC_REPUTATIONS: "NPC_REPUTATIONS",

  NAME_FEMALE_NAME: "NAME_FEMALE_NAME",
  NAME_MALE_NAME: "NAME_MALE_NAME",
  NAME_UPPER_CLASS_LAST_NAME: "NAME_UPPER_CLASS_LAST_NAME",
  NAME_LOWER_CLASS_LAST_NAME: "NAME_LOWER_CLASS_LAST_NAME",
  NAME_NICKNAME: "NAME_NICKNAME",

  CITY_THEMES: "CITY_THEMES",
  CITY_EVENTS: "CITY_EVENTS",
  CITY_DISTRICT_THEMES: "CITY_DISTRICT_THEMES",
  CITY_UPPER_CLASS_BUILDINGS: "CITY_UPPER_CLASS_BUILDINGS",
  CITY_LOWER_CLASS_BUILDINGS: "CITY_LOWER_CLASS_BUILDINGS",
  CITY_ACTIVITIES: "CITY_ACTIVITIES",
  CITY_BUILDING_ROOMS: "CITY_BUILDING_ROOMS",
  CITY_STREET_FEATURES: "CITY_STREET_FEATURES",
  CITY_BUILDING_FEATURE: "CITY_BUILDING_FEATURE",
  CITY_FACTIONS: "CITY_FACTIONS",
  CITY_FACTION_TRAITS: "CITY_FACTION_TRAITS",
  CITY_FACTION_GOALS: "CITY_FACTION_GOALS",

  DUNGEON_ENTRANCES: "DUNGEON_ENTRANCES",
  DUNGEON_FORMS: "DUNGEON_FORMS",
  DUNGEON_LAYOUT: "DUNGEON_LAYOUT",
  DUNGEON_RUINATIONS: "DUNGEON_RUINATIONS",
  DUNGEON_REWARDS: "DUNGEON_REWARDS",
  DUNGEON_ACTIVITIES: "DUNGEON_ACTIVITIES",
  DUNGEON_ROOMS: "DUNGEON_ROOMS",
  DUNGEON_ROOM_DETAILS: "DUNGEON_ROOM_DETAILS",
  DUNGEON_TRICKS: "DUNGEON_TRICKS",
  DUNGEON_HAZARDS: "DUNGEON_HAZARDS",
  DUNGEON_TRAP_EFFECTS: "DUNGEON_TRAP_EFFECTS",
  DUNGEON_TRAP_TRIGGERS: "DUNGEON_TRAP_TRIGGERS",

  ITEM_MISCELLANEOUS: "ITEM_MISCELLANEOUS",
  ITEM_WORN: "ITEM_WORN",
  ITEM_WEAPON: "ITEM_WEAPON",
  ITEM_BOOK_SUBJECT: "ITEM_BOOK_SUBJECT",
  ITEM_TOOL: "ITEM_TOOL",
  ITEM_POTIONS: "ITEM_POTIONS",
  ITEM_MAGICAL_INGREDIENT: "ITEM_MAGICAL_INGREDIENT",
  ITEM_TREASURE_ITEM: "ITEM_TREASURE_ITEM",
  ITEM_TREASURE_TRAIT: "ITEM_TREASURE_TRAIT",
  ITEM_VALUABLE_MATERIAL: "ITEM_VALUABLE_MATERIAL",

  WILD_REGION: "WILD_REGION",
  WILD_LANDMARKS: "WILD_LANDMARKS",
  WILD_STRUCTURES: "WILD_STRUCTURES",
  WILD_REGION_TRAITS: "WILD_REGION_TRAITS",
  WILD_DISCOVERIES: "WILD_DISCOVERIES",
  WILD_ACTIVITIES: "WILD_ACTIVITIES",
  WILD_HAZARDS: "WILD_HAZARDS",
  WILD_EDIBLE_PLANTS: "WILD_EDIBLE_PLANTS",
  WILD_POISONOUS_PLANTS: "WILD_POISONOUS_PLANTS",

  INN_ADJECTIVES: "INN_ADJECTIVES",
  INN_NOUNS: "INN_NOUNS",
  INN_QUIRKS: "INN_QUIRKS",

  CURSED: "CURSED",
  ALIGNMENT: "ALIGNMENT",
};
export const TABLE_NAMES_LIST = Object.values(TABLE_NAMES);

export const TABLES: { [s: string]: Table } = {
  [TABLE_NAMES.MAGIC_TABLE_ROLL]: {
    label: "Magic Spells: Table Roll",
    table: {
      "1": [
        "Physical Effect + Physical Form",
        "Physical Effect + Ethereal Form",
        "Ethereal Effect + Physical Form",
        "Ethereal Effect + Ethereal Form",
        "Physical Element + Physical Form",
        "Physical Element + Ethereal Form",
      ],
      "2": [
        "Ethereal Element + Physical Form",
        "Ethereal Element + Ethereal Form",
        "Physical Effect + Physical Element",
        "Physical Effect + Ethereal Element",
        "Ethereal Effect + Physical Element",
        "Ethereal Effect + Ethereal Element",
      ],
    },
  },
  [TABLE_NAMES.MAGIC_PHYSICAL_EFFECTS]: {
    label: "Magic Spells: Physical Effects",
    table: {
      "1": [
        "Animating",
        "Attracting",
        "Binding",
        "Blossoming",
        "Consuming",
        "Creeping",
      ],
      "2": [
        "Crushing",
        "Diminishing",
        "Dividing",
        "Duplicating",
        "Enveloping",
        "Expanding",
      ],
      "3": [
        "Fusing",
        "Grasping",
        "Hastening",
        "Hindering",
        "Illuminating",
        "Imprisoning",
      ],
      "4": [
        "Levitating",
        "Opening",
        "Petrifying",
        "Phasing",
        "Piercing",
        "Pursuing",
      ],
      "5": [
        "Reflecting",
        "Regenerating",
        "Rending",
        "Repelling",
        "Resurrecting",
        "Screaming",
      ],
      "6": [
        "Sealing",
        "Shapeshifting",
        "Shielding",
        "Spawning",
        "Transmuting",
        "Transporting",
      ],
    },
  },
  [TABLE_NAMES.MAGIC_PHYSICAL_ELEMENTS]: {
    label: "Magic Spells: Physical Elements",
    table: {
      "1": ["Acid", "Amber", "Bark", "Blood", "Bone", "Brine"],
      "2": ["Clay", "Crow", "Crystal", "Ember", "Flesh", "Fungus"],
      "3": ["Glass", "Honey", "Ice", "Insect", "Wood", "Lava"],
      "4": ["Moss", "Obsidian", "Oil", "Poison", "Rat", "Salt"],
      "5": ["Sand", "Sap", "Serpent", "Slime", "Stone", "Tar"],
      "6": ["Thorn", "Vine", "Water", "Wine", "Wood", "Worm"],
    },
  },
  [TABLE_NAMES.MAGIC_PHYSICAL_FORMS]: {
    label: "Magic Spells: Physical Forms",
    valueSeparator: " ",
    table: {
      "1": ["Altar", "Armor", "Arrow", "Beast", "Blade", "Cauldron"],
      "2": ["Chain", "Chariot", "Claw", "Cloak", "Colossus", "Crown"],
      "3": ["Elemental", "Eye", "Fountain", "Gate", "Golem", "Hammer"],
      "4": ["Horn", "Key", "Mask", "Monolith", "Pit", "Prison"],
      "5": ["Sentinel", "Servant", "Shield", "Spear", "Steed", "Swarm"],
      "6": ["Tentacle", "Throne", "Torch", "Trap", "Wall", "Web"],
    },
  },
  [TABLE_NAMES.MAGIC_ETHEREAL_EFFECTS]: {
    label: "Magic Spells: Ethereal Effects",
    table: {
      "1": [
        "Avenging",
        "Banishing",
        "Bewildering",
        "Blinding",
        "Charming",
        "Communicating",
      ],
      "2": [
        "Compelling",
        "Concealing",
        "Deafening",
        "Deceiving",
        "Deciphering",
        "Disguising",
      ],
      "3": [
        "Dispelling",
        "Emboldening",
        "Encoding",
        "Energizing",
        "Enlightening",
        "Enraging",
      ],
      "4": [
        "Excruciating",
        "Foreseeing",
        "Intoxicating",
        "Maddening",
        "Mesmerizing",
        "Mindreading",
      ],
      "5": [
        "Nullifying",
        "Paralyzing",
        "Revealing",
        "Revolting",
        "Scrying",
        "Silencing",
      ],
      "6": [
        "Soothing",
        "Summoning",
        "Terrifying",
        "Warding",
        "Wearying",
        "Withering",
      ],
    },
  },
  [TABLE_NAMES.MAGIC_ETHEREAL_ELEMENTS]: {
    label: "Magic Spells: Ethereal Elements",
    table: {
      "1": ["Ash", "Chaos", "Distortion", "Dream", "Dust", "Echo"],
      "2": ["Ectoplasm", "Fire", "Fog", "Ghost", "Harmony", "Heat"],
      "3": ["Light", "Lightning", "Memory", "Mind", "Mutation", "Negation"],
      "4": ["Plague", "Plasma", "Probability", "Rain", "Rot", "Shadow"],
      "5": ["Smoke", "Snow", "Soul", "Star", "Stasis", "Steam"],
      "6": ["Thunder", "Time", "Void", "Warp", "Whisper", "Wind"],
    },
  },
  [TABLE_NAMES.MAGIC_ETHEREAL_FORMS]: {
    label: "Magic Spells: Ethereal Forms",
    valueSeparator: " ",
    table: {
      "1": ["Aura", "Beacon", "Beam", "Blast", "Blob", "Bolt"],
      "2": ["Bubble", "Call", "Cascade", "Circle", "Cloud", "Coil"],
      "3": ["Cone", "Cube", "Dance", "Disk", "Field", "Form"],
      "4": ["Gaze", "Loop", "Moment", "Nexus", "Portal", "Pulse"],
      "5": ["Pyramid", "Ray", "Shard", "Sphere", "Spray", "Storm"],
      "6": ["Swarm", "Torrent", "Touch", "Vortex", "Wave", "Word"],
    },
  },
  [TABLE_NAMES.MAGIC_MUTATIONS]: {
    label: "Magic: Catastrophes",
    table: {
      "1": [
        "Ages",
        "Attracts birds",
        "Child-form",
        "Corpulence",
        "Covered in hair",
        "Animal arms",
      ],
      "2": [
        "Animal eyes",
        "Animal head",
        "Animal legs",
        "Animal mouth",
        "Animal skin",
        "Animal form",
      ],
      "3": [
        "Cyclops",
        "Extra arms",
        "Extra eyes",
        "Extra legs",
        "Forked tongue",
        "Genderswap",
      ],
      "4": [
        "Hunchback",
        "Item form",
        "Long arms",
        "Lose all hair",
        "Lose teeth",
        "Monster features",
      ],
      "5": [
        "Monster Trait",
        "No eyes",
        "No mouth",
        "Physical elementskin",
        "Second face",
        "Sheds skin",
      ],
      "6": [
        "Shrinks",
        "Shrivels",
        "Skin boils",
        "Slime trail",
        "Translucent skin",
        "Weeps blood",
      ],
    },
  },
  [TABLE_NAMES.MAGIC_INSANITIES]: {
    label: "Magic: Insanities",
    table: {
      "1": [
        "Always lies",
        "Always polite",
        "Animal form",
        "Cannot count",
        "Cannot lie",
        "Faceblind",
      ],
      "2": [
        "Fears birds",
        "Fears blood",
        "Fears books",
        "Fears darkness",
        "Fears fire",
        "Fears gold",
      ],
      "3": [
        "Fears horses",
        "Fears iron",
        "Fears music",
        "Fears own hand",
        "Fears PC",
        "Fears rain",
      ],
      "4": [
        "Fears rivers",
        "Fears silence",
        "Fears sleep",
        "Fears sunlight",
        "Fears the moon",
        "Fears trees",
      ],
      "5": [
        '"Genius"',
        '"Gorgeous"',
        "Hates violence",
        '"Invisible"',
        '"Invulnerable"',
        '"Monster ability"',
      ],
      "6": [
        '"Monster feature"',
        '"Monster trait"',
        "Must sing",
        "New personality",
        "Says thoughts",
        "Sees dead people",
      ],
    },
  },
  [TABLE_NAMES.MAGIC_CATASTROPHES]: {
    label: "Magic: Catastrophes",
    table: {
      "1": [
        "All iron rusts",
        "Animals die",
        "Animals mutate",
        "Birds attack",
        "City appears",
        "Deadly fog",
      ],
      "2": [
        "Dream plague",
        "Endless night",
        "Endless rain",
        "Endless storm",
        "Endless twilight",
        "Endless winter",
      ],
      "3": [
        "Fae return",
        "Forest appears",
        "Forgetfulness",
        "Graves open",
        "Lamentations",
        "Maggots",
      ],
      "4": [
        "Mass insanity",
        "Mass mutation",
        "Mass slumber",
        "Meteor strike",
        "Mirrors speak",
        "No stars",
      ],
      "5": [
        "Outsider enters",
        "People shrink",
        "People vanish",
        "Plants wither",
        "Portal opens",
        "Rifts open",
      ],
      "6": [
        "Shadows speak",
        "Space distorts",
        "Stones speak",
        "Total silence",
        "Tower appears",
        "Water to blood",
      ],
    },
  },
  [TABLE_NAMES.CHARACTER_SEX]: {
    label: "Character: Sex",
    table: {
      "1": ["Male"],
      "2": ["Female"],
      "3": ["Hard to tell"],
    },
  },
  [TABLE_NAMES.CHARACTER_RACE]: {
    label: "Character: Race",
    table: {
      "1": ["Dragonborn", "Dwarf", "Elf", "Gnome"],
      "2": ["Half-elf", "Halfling", "Half-orc", "Human"],
      "3": ["Tiefling", "Bugbear", "Genasi", "Harengon"],
      "4": ["Hobgoblin", "Lizardfolk", "Minotaur", "Tabaxi"],
    },
  },
  [TABLE_NAMES.CHARACTER_APPEARANCE]: {
    label: "Character: Build",
    defaultNumRolls: 2,
    table: {
      "1": [
        "Aquiline",
        "Athletic",
        "Barrel-Chested",
        "Boney",
        "Brawny",
        "Brutish",
      ],
      "2": [
        "Bullnecked",
        "Chiseled",
        "Coltish",
        "Corpulent",
        "Craggy",
        "Delicate",
      ],
      "3": ["Furrowed", "Gaunt", "Gorgeous", "Grizzled", "Haggard", "Handsome"],
      "4": ["Hideous", "Lanky", "Pudgy", "Ripped", "Rosy", "Scrawny"],
      "5": [
        "Sinewy",
        "Slender",
        "Slumped",
        "Solid",
        "Square-Jawed",
        "Statuesque",
      ],
      "6": ["Towering", "Trim", "Weathered", "Willowy", "Wiry", "Wrinkled"],
    },
  },
  [TABLE_NAMES.CHARACTER_PHYSICAL_DETAIL]: {
    label: "Character: Physical Detail",
    defaultNumRolls: 2,
    table: {
      "1": [
        "Acid scars",
        "Battle scars",
        "Birthmark",
        "Braided hair",
        "Brand mark",
        "Broken nose",
      ],
      "2": [
        "Bronze skinned",
        "Burn scars",
        "Bushy eyebrows",
        "Curly hair",
        "Dark skinned",
        "Dreadlocks",
      ],
      "3": [
        "Exotic accent",
        "Flogging scars",
        "Freckles",
        "Gold tooth",
        "Hoarse voice",
        "Huge beard",
      ],
      "4": [
        "Long hair",
        "Matted hair",
        "Missing ear",
        "Missing teeth",
        "Mustache",
        "Muttonchops",
      ],
      "5": [
        "Nine fingers",
        "Oiled hair",
        "One-eyed",
        "Pale skinned",
        "Piercings",
        "Ritual scars",
      ],
      "6": [
        "Sallow skin",
        "Shaved head",
        "Sunburned",
        "Tangled Hair",
        "Tattoos",
        "Topknot",
      ],
    },
  },
  [TABLE_NAMES.CHARACTER_CLOTHES]: {
    label: "Character: Clothes",
    defaultNumRolls: 2,
    table: {
      "1": [
        "Antique",
        "Battle-torn",
        "Bedraggled",
        "Blood-stained",
        "Ceremonial",
        "Dated",
      ],
      "2": [
        "Decaying",
        "Eccentric",
        "Elegant",
        "Embroidered",
        "Exotic",
        "Fashionable",
      ],
      "3": [
        "Flamboyant",
        "Food-stained",
        "Formal",
        "Frayed",
        "Frumpy",
        "Garish",
      ],
      "4": [
        "Grimy",
        "Haute couture",
        "Lacey",
        "Livery",
        "Mud-stained",
        "Ostentatious",
      ],
      "5": [
        "Oversized",
        "Patched",
        "Patterned",
        "Perfumed",
        "Practical",
        "Rumpled",
      ],
      "6": [
        "Sigils",
        "Singed",
        "Tasteless",
        "Undersized",
        "Wine-stained",
        "Worn out",
      ],
    },
  },
  [TABLE_NAMES.CHARACTER_PERSONALITY]: {
    label: "Character: Personality",
    table: {
      "1": ["Bitter", "Brave", "Cautious", "Chipper", "Contrary", "Cowardly"],
      "2": [
        "Cunning",
        "Driven",
        "Entitled",
        "Gregarious",
        "Grumpy",
        "Heartless",
      ],
      "3": [
        "Honor-bound",
        "Hotheaded",
        "Inquisitive",
        "Irascible",
        "Jolly",
        "Know-it-all",
      ],
      "4": ["Lazy", "Loyal", "Menacing", "Mopey", "Nervous", "Protective"],
      "5": ["Righteous", "Rude", "Sarcastic", "Savage", "Scheming", "Serene"],
      "6": [
        "Spacey",
        "Stoic",
        "Stubborn",
        "Stuck-up",
        "Suspicious",
        "Wisecracking",
      ],
    },
  },
  [TABLE_NAMES.CHARACTER_MANNERISM]: {
    label: "Character: Mannerism",
    table: {
      "1": [
        "Anecdotes",
        "Breathy",
        "Chuckles",
        "Clipped",
        "Cryptic",
        "Deep voice",
      ],
      "2": [
        "Drawl",
        "Enunciates",
        "Flowery speech",
        "Gravelly voice",
        "Highly formal",
        "Hypnotic",
      ],
      "3": [
        "Interrupts",
        "Laconic",
        "Laughs",
        "Long pauses",
        "Melodious",
        "Monotone",
      ],
      "4": [
        "Mumbles",
        "Narrates",
        "Overly casual",
        "Quaint sayings",
        "Rambles",
        "Random facts",
      ],
      "5": [
        "Rapid-fire",
        "Rhyming",
        "Robotic",
        "Slow speech",
        "Speechifies",
        "Squeaky",
      ],
      "6": [
        "Street slang",
        "Stutters",
        "Talks to self",
        "Trails off",
        "Very loud",
        "Whispers",
      ],
    },
  },
  [TABLE_NAMES.GOTHIC_BACKGROUND]: {
    label: "Character: Gothic Background",
    table: {
      "1": [
        "Alchemist",
        "Beggar-prince",
        "Blackmailer",
        "Bounty-hunter",
        "Chimney sweep",
        "Coin-clipper",
      ],
      "2": [
        "Contortionist",
        "Counterfeiter",
        "Cultist",
        "Cutpurse",
        "Debt-collector",
        "Deserter",
      ],
      "3": [
        "Fence",
        "Fortuneteller",
        "Galley slave",
        "Gabler",
        "Gravedigger",
        "Headsman",
      ],
      "4": [
        "Hedge knight",
        "Highwayman",
        "Housebreaker",
        "Kidnapper",
        "Mad prophet",
        "Mountebank",
      ],
      "5": [
        "Peddler",
        "Pit-fighter",
        "Poisoner",
        "Rat-catcher",
        "Scrivener",
        "Sellsword",
      ],
      "6": [
        "Slave",
        "Smuggler",
        "Street performer",
        "Tattooist",
        "Urchin",
        "Usurer",
      ],
    },
  },
  [TABLE_NAMES.MONSTER_TYPE]: {
    label: "Monster: Table Roll",
    table: {
      "1": ["Aerial"],
      "2": ["Terrestrial"],
      "3": ["Aquatic"],
    },
  },
  [TABLE_NAMES.MONSTER_AERIAL_FORM]: {
    label: "Monster: Aerial Animals",
    table: {
      "1": [
        "Albatross",
        "Bat",
        "Beetle",
        "Bird of Paradise",
        "Butterfly",
        "Condor",
      ],
      "2": ["Crane", "Crow", "Dragonfly", "Eagle", "Falcon", "Firefly"],
      "3": [
        "Flamingo",
        "Fly",
        "Flying Squirrel",
        "Goose",
        "Gull",
        "Hummingbird",
      ],
      "4": [
        "Kingfisher",
        "Locust",
        "Magpie",
        "Mantis",
        "Mockingbird",
        "Mosquito",
      ],
      "5": ["Moth", "Owl", "Parrot", "Peacock", "Pelican", "Pterandon"],
      "6": ["Rooster", "Sparrow", "Swan", "Vulture", "Wasp", "Woodpecker"],
    },
  },
  [TABLE_NAMES.MONSTER_TERRESTRIAL_FORM]: {
    label: "Monster: Terrestrial Animals",
    table: {
      "1": ["Ant", "Ape", "Armadillo", "Badger", "Bear", "Boar"],
      "2": [
        "Caterpillar",
        "Centipede",
        "Chameleon",
        "Cockroach",
        "Deer",
        "Elephant",
      ],
      "3": ["Ferret", "Fox", "Giraffe", "Goat", "Horse", "Human"],
      "4": ["Mole", "Ostrich", "Ox", "Porcupine", "Rabbit", "Raccoon"],
      "5": ["Rat", "Rhinoceros", "Scorpion", "Sheep", "Slug", "Snail"],
      "6": ["Snake", "Spider", "Squirrel", "Tiger", "Wolf", "Wolverine"],
    },
  },
  [TABLE_NAMES.MONSTER_AQUATIC_FORM]: {
    label: "Monster: Aquatic Animals",
    table: {
      "1": ["Alligator", "Amoeba", "Angler", "Beaver", "Clam", "Crab"],
      "2": ["Dolphin", "Eel", "Frog", "Hippopotamus", "Jellyfish", "Leech"],
      "3": ["Lobster", "Manatee", "Manta Ray", "Muskrat", "Narwhal", "Newt"],
      "4": [
        "Octopus",
        "Otter",
        "Penguin",
        "Platypus",
        "Pufferfish",
        "Salamander",
      ],
      "5": ["Sea Anemone", "Sea Urchin", "Seahorse", "Seal", "Shark", "Shrimp"],
      "6": ["Squid", "Swordfish", "Tadpole", "Turtle", "Walrus", "Whale"],
    },
  },
  [TABLE_NAMES.MONSTER_FEATURES]: {
    label: "Monster: Features",
    table: {
      "1": [
        "Antlers",
        "Beak",
        "Carapace",
        "Claws",
        "Compound eyes",
        "Eye Stalks",
      ],
      "2": ["Fangs", "Fins", "Fur", "Gills", "Hooves", "Horns"],
      "3": [
        "Legless",
        "Long tongue",
        "Many-eyed",
        "Many-limbed",
        "Mucus",
        "Pincers",
      ],
      "4": [
        "Plates",
        "Plumage",
        "Proboscis",
        "Scales",
        "Segmented",
        "Shaggy hair",
      ],
      "5": [
        "Shell",
        "Spikes",
        "Spinerets",
        "Spines",
        "Stinger",
        "Suction cups",
      ],
      "6": ["Tail", "Talons", "Tentacles", "Trunk", "Tusks", "Wings"],
    },
  },
  [TABLE_NAMES.MONSTER_TRAITS]: {
    label: "Monster: Traits",
    table: {
      "1": [
        "Amphibious",
        "Bloated",
        "Brittle",
        "Cannibal",
        "Clay-like",
        "Colossal",
      ],
      "2": [
        "Crystalline",
        "Decaying",
        "Ethereal Element",
        "Ethereal",
        "Ever-young",
        "Eyeless",
      ],
      "3": [
        "Fearless",
        "Fluffy",
        "Fungal",
        "Gelatinous",
        "Geometric",
        "Hardened",
      ],
      "4": [
        "Illusory",
        "Intelligent",
        "Iridescent",
        "Luminous",
        "Many-headed",
        "Mechanical",
      ],
      "5": [
        "Physical Element",
        "Planar",
        "Reflective",
        "Rubbery",
        "Shadowy",
        "Sharp",
      ],
      "6": ["Skeletal", "Slimy", "Sticky", "Stinking", "Tiny", "Translucent"],
    },
  },
  [TABLE_NAMES.MONSTER_ABILITIES]: {
    label: "Monster: Abilities",
    table: {
      "1": [
        "Absorbing",
        "Acid blood",
        "Anti-magic",
        "Blinding",
        "Breath weapon",
        "Camouflaging",
      ],
      "2": [
        "Duplicating",
        "Electric",
        "Entangilng",
        "Ethereal Effect",
        "Exploding",
        "Flying",
      ],
      "3": [
        "Gaze weapon",
        "Hypnotizing",
        "Impervious",
        "Invisible",
        "Life-draining",
        "Magnetic",
      ],
      "4": [
        "Mimicking",
        "Mind-Reading",
        "Paralyzing",
        "Phasing",
        "Physical Effect",
        "Poisonous",
      ],
      "5": [
        "Radioactive",
        "Reflective",
        "Regenerating",
        "Shapeshifting",
        "Spell-casting",
        "Stealthy",
      ],
      "6": [
        "Strangling",
        "Super-strength",
        "Telekinetic",
        "Teleporting",
        "Vampiric",
        "Wall-Crawling",
      ],
    },
  },
  [TABLE_NAMES.MONSTER_TACTICS]: {
    label: "Monster: Tactics",
    table: {
      "1": [
        "Ambush",
        "Call for support",
        "Capture",
        "Charge",
        "Climb foes",
        "Compel Worship",
      ],
      "2": [
        "Create barrier",
        "Deceive",
        "Demand duel",
        "Disorient",
        "Encircle",
        "Evade",
      ],
      "3": [
        "Gang up",
        "Gather strength",
        "Go berserk",
        "Harry",
        "Hurl foes",
        "Immobilize",
      ],
      "4": [
        "Manipulate",
        "Mock",
        "Monologue",
        "Order minion",
        "Protect leader",
        "Protect self",
      ],
      "5": [
        "Scatter foes",
        "Stalk",
        "Steal from",
        "Swarm",
        "Target insolent",
        "Target leader",
      ],
      "6": [
        "Target nearest",
        "Target richest",
        "Target strongest",
        "Target weakest",
        "Toy with",
        "Use terrain",
      ],
    },
  },
  [TABLE_NAMES.MONSTER_PERSONALITY]: {
    label: "Monster: Personality",
    table: {
      "1": ["Alien", "Aloof", "Bored", "Cautious", "Cowardly", "Curious"],
      "2": [
        "Devious",
        "Distractible",
        "Educated",
        "Embittered",
        "Envious",
        "Erudite",
      ],
      "3": [
        "Fanatical",
        "Forgetful",
        "Generous",
        "Hateful",
        "Honorable",
        "Humble",
      ],
      "4": [
        "Jaded",
        "Jovial",
        "Legalistic",
        "Manipulative",
        "Megalomaniac",
        "Melancholy",
      ],
      "5": [
        "Meticulous",
        "Mystical",
        "Obsessive",
        "Out of Touch",
        "Paranoid",
        "Polite",
      ],
      "6": [
        "Psychopathic",
        "Sophisticated",
        "Touchy",
        "Unimpressed",
        "Vain",
        "Xenophobic",
      ],
    },
  },
  [TABLE_NAMES.MONSTER_WEAKNESS]: {
    label: "Monster: Weaknesses",
    table: {
      "1": [
        "Bells",
        "Birdsong",
        "Children",
        "Cold",
        "Cold Iron",
        "Competition",
      ],
      "2": [
        "Conversation",
        "Deformity",
        "Flattery",
        "Flowers",
        "Gifts",
        "Gold",
      ],
      "3": [
        "Heat",
        "Holy Icon",
        "Holy Water",
        "Home Cooking",
        "Insanities",
        "Mirrors",
      ],
      "4": [
        "Mistletoe",
        "Moonlight",
        "Music",
        "Methods",
        "Phylactery",
        "Physical Element",
      ],
      "5": ["Puzzles", "Riddles", "Rituals", "Silver", "Sunlight", "Tears"],
      "6": [
        "True Name",
        "Valuable Materials",
        "Weak Spot",
        "Weapon Items",
        "Wine",
        "Wormwood",
      ],
    },
  },
  [TABLE_NAMES.NPC_OCCUPATION_CIVILIZATION]: {
    label: "NPC: Civilized Occupation",
    table: {
      "1": ["Acolyte", "Actor", "Apothecary", "Baker", "Barber", "Blacksmith"],
      "2": [
        "Brewer",
        "Bureaucrat",
        "Butcher",
        "Carpenter",
        "Clockmaker",
        "Courier",
      ],
      "3": [
        "Courtier",
        "Diplomat",
        "Fishmonger",
        "Guard",
        "Haberdasher",
        "Innkeeper",
      ],
      "4": ["Item-seller", "Jeweler", "Knight", "Locksmith", "Mason", "Miller"],
      "5": ["Musician", "Noble", "Painter", "Priest", "Scholar", "Scribe"],
      "6": [
        "Sculptor",
        "Shipwright",
        "Soldier",
        "Tailor",
        "Taxidermist",
        "Wigmaker",
      ],
    },
  },
  [TABLE_NAMES.NPC_OCCUPATION_UNDERWORLD]: {
    label: "NPC: Underworld Occupation",
    table: {
      "1": [
        "Alchemist",
        "Beggar-prince",
        "Blackmailer",
        "Bounty-hunter",
        "Chimney sweep",
        "Coin-clipper",
      ],
      "2": [
        "Contortionist",
        "Counterfeiter",
        "Cultist",
        "Cutpurse",
        "Debt-collector",
        "Deserter",
      ],
      "3": [
        "Fence",
        "Fortuneteller",
        "Galley slave",
        "Gambler",
        "Gravedigger",
        "Headsman",
      ],
      "4": [
        "Hedge knight",
        "Highwayman",
        "Housebreaker",
        "Kidnapper",
        "Mad prophet",
        "Mountebank",
      ],
      "5": [
        "Peddler",
        "Pit-fighter",
        "Poisoner",
        "Rat-catcher",
        "Scrivener",
        "Sellsword",
      ],
      "6": [
        "Slave",
        "Smuggler",
        "Street performer",
        "Tattooist",
        "Urchin",
        "Usurer",
      ],
    },
  },
  [TABLE_NAMES.NPC_OCCUPATION_WILDERNESS]: {
    label: "NPC: Wilderness Occupation",
    table: {
      "1": [
        "Apiarist",
        "Bandit",
        "Caravan Guard",
        "Caravaneer",
        "Druid",
        "Exile",
      ],
      "2": [
        "Explorer",
        "Farmer",
        "Fisherman",
        "Forager",
        "Fugitive",
        "Hedge wizard",
      ],
      "3": [
        "Hermit",
        "Hunter",
        "Messenger",
        "Minstrel",
        "Monk",
        "Monster hunter",
      ],
      "4": ["Outlander", "Tinker", "Pilgrim", "Poacher", "Raider", "Ranger"],
      "5": ["Sage", "Scavenger", "Scout", "Shepher", "Seer", "Surveyor"],
      "6": [
        "Tinker",
        "Tomb raider",
        "Trader",
        "Trapper",
        "Witch",
        "Woodcutter",
      ],
    },
  },
  [TABLE_NAMES.NAME_FEMALE_NAME]: {
    label: "Name: Female Name",
    table: {
      "1": ["Adelaide", "Alma", "Barsaba", "Beatrix", "Bianca", "Cleopha"],
      "2": ["Clover", "Constance", "Damaris", "Daphne", "Demona", "Elsbeth"],
      "3": ["Esme", "Fern", "Hester", "Hippolyta", "Jessamine", "Jilly"],
      "4": ["Morgot", "Minerva", "Nerissa", "Odetta", "Olga", "Orchid"],
      "5": ["Pepper", "Phoebe", "Piety", "Poppy", "Silence", "Sybil"],
      "6": ["Trillby", "Tuesday", "Ursula", "Vivian", "Wendy", "Zora"],
    },
  },
  [TABLE_NAMES.NAME_MALE_NAME]: {
    label: "Name: Male Name",
    table: {
      "1": ["Balthazar", "Basil", "Bertram", "Blaxton", "Chadwick", "Clovis"],
      "2": ["Destrian", "Ellis", "Erasmus", "Faustus", "Finn", "Fitzhugh"],
      "3": ["Florian", "Fox", "Godwin", "Hannibal", "Jasper", "Jiles"],
      "4": ["Jules", "Leopold", "Merrick", "Mortimer", "Ogden", "Orion"],
      "5": [
        "Oswald",
        "Percival",
        "Peregrine",
        "Quentin",
        "Redmain",
        "Reinhold",
      ],
      "6": ["Silas", "Stilton", "Stratford", "Tenpiece", "Waverly", "Webster"],
    },
  },
  [TABLE_NAMES.NAME_UPPER_CLASS_LAST_NAME]: {
    label: "Name: Upper Class Last Name",
    table: {
      "1": ["Belvedere", "Bithesea", "Calaver", "Carvolo", "De Rippe", "Droll"],
      "2": ["Dunlow", "Edevane", "Erelong", "Febland", "Fernsby", "Fisk"],
      "3": [
        "Gastrell",
        "Girdwood",
        "Gorgon",
        "Grimeson",
        "Gruger",
        "Hitheryon",
      ],
      "4": [
        "La Marque",
        "Malmora",
        "Miter",
        "Oblington",
        "Onymous",
        "Phillifent",
      ],
      "5": [
        "Portendorfer",
        "Romatet",
        "Rothery",
        "Skorbeck",
        "Slora",
        "Southwark",
      ],
      "6": [
        "Stavish",
        "Vandermeer",
        "Wellbelove",
        "Westergren",
        "Wexley",
        "Wilberforce",
      ],
    },
  },
  [TABLE_NAMES.NAME_LOWER_CLASS_LAST_NAME]: {
    label: "Name: Lower Class Last Name",
    table: {
      "1": [
        "Barrow",
        "Beetleman",
        "Berrycloth",
        "Birdwhistle",
        "Bobich",
        "Chips",
      ],
      "2": [
        "Coffin",
        "Crumpling",
        "Culpepper",
        "Dankworth",
        "Digworthy",
        "Dreggs",
      ],
      "3": ["Gimble", "Gravework", "Greelish", "Hardwick", "Hatman", "Hovel"],
      "4": ["Knibbs", "Midnighter", "Needle", "Nethercoat", "Pestle", "Relish"],
      "5": [
        "Rumbold",
        "Rummage",
        "Sallow",
        "Saltmarsh",
        "Silverless",
        "Skitter",
      ],
      "6": ["Slee", "Slitherly", "Stoker", "Tarwater", "Tumbler", "Villin"],
    },
  },
  [TABLE_NAMES.NAME_NICKNAME]: {
    label: "Name: Nickname",
    table: {
      "1": ["Grim", "Hog", "Nobody", "Danger", "Jester", "Butcher"],
      "2": ["Hound", "Fierce", "Champ", "Killer", "Tweaker", "Puffer"],
      "3": ["Scalpel", "Lumps", "Spoon", "Rambler", "Slug", "Porky"],
      "4": ["Cookie", "Buster", "Shade", "Family member", "Mugs", "Chef"],
      "5": ["Bull", "Babe", "Animal type", "Glide", "Gemstone", "Doc"],
      "6": [
        "Belle",
        "Skip",
        "Lucky",
        "Buddy",
        "Physical Element",
        "Ethereal Element",
      ],
    },
  },
  [TABLE_NAMES.NPC_ASSETS]: {
    label: "NPC: Assets",
    table: {
      "1": [
        "Authority",
        "Avoids Detection",
        "Calls in favors",
        "Charming",
        "Cooks the books",
        "Erases the evidence",
      ],
      "2": [
        "Excellent liar",
        "Extremely rich",
        "Faction-leader",
        "Faction-member",
        "Feared",
        "Fortified base",
      ],
      "3": [
        "Gorgeous",
        "Hears rumors",
        "Huge family",
        "Huge library",
        "Impersonator",
        "Interrogator",
      ],
      "4": [
        "Knows a guy",
        "Knows a way in",
        "Launders money",
        "Learned",
        "Local celebrity",
        "Local knowledge",
      ],
      "5": [
        "Loyal henchmen",
        "Middling oracle",
        "Nothing to lose",
        "Own the guards",
        "Powerful spouse",
        "Procures gear",
      ],
      "6": [
        "Pulls the strings",
        "Secret lab",
        "Sells contraband",
        "Smuggles goods",
        "Spy network",
        "War hero",
      ],
    },
  },
  [TABLE_NAMES.NPC_LIABILITIES]: {
    label: "NPC: Liabilities",
    table: {
      "1": [
        "Addicted",
        "Alcoholic",
        "Corrupt ally",
        "Coward",
        "Decadent",
        "Forbidden love",
      ],
      "2": [
        "Gambler",
        "Glutton",
        "Greedy",
        "Heretical",
        "Huge debts",
        "Imposter",
      ],
      "3": [
        "Insane",
        "Jealous",
        "Leave evidence",
        "Many enemies",
        "Misinformed",
        "Money trail",
      ],
      "4": [
        "Narcissist",
        "Need medicine",
        "OCD",
        "Paranoid",
        "Partyer",
        "Poor equipment",
      ],
      "5": [
        "Protective",
        "Scandalous",
        "Softhearted",
        "Strict routines",
        "Superstitious",
        "Suspicious",
      ],
      "6": [
        "Temper",
        "Trusting",
        "Vulnerable base",
        "Wanted",
        "Weak-willed",
        "Widely despised",
      ],
    },
  },
  [TABLE_NAMES.NPC_GOALS]: {
    label: "NPC: Goals",
    table: {
      "1": [
        "A better life",
        "Acceptance",
        "Acquire an item",
        "Craft an item",
        "Destroy a faction",
        "Destroy an item",
      ],
      "2": [
        "Enlightenment",
        "Fame",
        "Find a faction",
        "Freedom",
        "Glory",
        "Impress a NPC",
      ],
      "3": [
        "Infamy",
        "Infiltrate a faction",
        "Justice",
        "Kidnap NPC",
        "Lead a faction",
        "Learning",
      ],
      "4": [
        "Locate a NPC",
        "Love",
        "Mastery",
        "Power",
        "Reach a location",
        "Rescue a NPC",
      ],
      "5": [
        "Resolve a dispute",
        "Restore a faction",
        "Reveal a secret",
        "Revenge",
        "Sabotage a faction",
        "Serve a deity",
      ],
      "6": [
        "Serve evil",
        "Serve a faction",
        "Serve an ideology",
        "Serve a leader",
        "Serve the needy",
        "Procure Wealth",
      ],
    },
  },
  [TABLE_NAMES.NPC_MISFORTUNES]: {
    label: "NPC: Misfortunes",
    table: {
      "1": [
        "Abandoned",
        "Addicted",
        "Arrested",
        "Blackmailed",
        "Burgled",
        "Challenged",
      ],
      "2": [
        "Condemned",
        "Crippled",
        "Cursed",
        "Defrauded",
        "Demoted",
        "Depressed",
      ],
      "3": [
        "Discredited",
        "Dismissed",
        "Disowned",
        "Exiled",
        "Famished",
        "Forgotten",
      ],
      "4": [
        "Framed",
        "Haunted",
        "Humiliated",
        "Impoverished",
        "Kidnapped",
        "Lost",
      ],
      "5": [
        "Mobbed",
        "Mutilated",
        "Overworked",
        "Poisoned",
        "Pursued",
        "Rejected",
      ],
      "6": ["Replaced", "Robbed", "Sick", "Sued", "Suspected", "Transformed"],
    },
  },
  [TABLE_NAMES.NPC_MISSIONS]: {
    label: "NPC: MIssions",
    table: {
      "1": [
        "Apprehend",
        "Assassinate",
        "Blackmail",
        "Burgle",
        "Chart",
        "Convince",
      ],
      "2": ["Deface", "Defraud", "Deliver", "Destroy", "Discredit", "Escort"],
      "3": [
        "Exfiltrate",
        "Extort",
        "Follow",
        "Frame",
        "Impersonate",
        "Impress",
      ],
      "4": [
        "Infiltrate",
        "Interrogate",
        "Investigate",
        "Kidnap",
        "Locate",
        "Plant",
      ],
      "5": ["Protect", "Raid", "Replace", "Retrieve", "Rob", "Ruin"],
      "6": [
        "Sabotage",
        "Smuggle",
        "Surveil",
        "Take over",
        "Terrorize",
        "Threaten",
      ],
    },
  },
  [TABLE_NAMES.NPC_REPUTATIONS]: {
    label: "NPC: Reputations",
    table: {
      "1": [
        "Ambitious",
        "Authoritative",
        "Boor",
        "Borrower",
        "Celebrity",
        "Charitable",
      ],
      "2": [
        "Cheat",
        "Dangerous",
        "Entertainer",
        "Gossip",
        "Hardworking",
        "Holy",
      ],
      "3": [
        "Honest",
        "Hypochondriac",
        "Idiot",
        "Influential",
        "Layabout",
        "Leader",
      ],
      "4": [
        "Miasnthrope",
        "Miser",
        "Neighborly",
        "Nutjob",
        "Obnoxious",
        "Overeducated",
      ],
      "5": [
        "Partier",
        "Pious",
        "Proper",
        "Prophet of doom",
        "Repulsive",
        "Respected",
      ],
      "6": [
        "Riffraff",
        "Scandalous",
        "Slime ball",
        "Terrifying",
        "Weirdo",
        "Wise",
      ],
    },
  },
  [TABLE_NAMES.CITY_THEMES]: {
    label: "City: Themes",
    table: {
      "1": [
        "Animal",
        "Aristocracy",
        "Art",
        "Bureaucracy",
        "Castes",
        "Catacombs",
      ],
      "2": [
        "City Activity",
        "City Event",
        "Crime families",
        "Cruelty",
        "District Theme",
        "Divine Domain",
      ],
      "3": [
        "Faction",
        "Festivals",
        "Feuds",
        "Intrigue",
        "Low Class Building",
        "Martial Law",
      ],
      "4": [
        "Meritocracy",
        "NPC",
        "Opulence",
        "Physical Element",
        "Pilgrimages",
        "Piracy",
      ],
      "5": [
        "Plutocracy",
        "Poverty",
        "Rituals",
        "Slavery",
        "Spices",
        "Theocracy",
      ],
      "6": [
        "Thievery",
        "Trade",
        "Tyranny",
        "Upper Class Building",
        "Wizardry",
        "Xenophobia",
      ],
    },
  },
  [TABLE_NAMES.CITY_EVENTS]: {
    label: "City: Events",
    table: {
      "1": [
        "Assassination",
        "Carnival",
        "Conscription",
        "Coronation",
        "Coup",
        "Cult activity",
      ],
      "2": [
        "Curfew",
        "Discovery",
        "Earthquake",
        "Faction war",
        "Fashion trend",
        "Fire",
      ],
      "3": [
        "Flood",
        "Heavy fog",
        "Heavy taxes",
        "Holy day",
        "Hysteria",
        "Inquisition",
      ],
      "4": [
        "Insurrection",
        "Invasion",
        "Jailbreak",
        "Mass eviction",
        "Mass pardon",
        "Negotiations",
      ],
      "5": [
        "Plague",
        "Proclamation",
        "Prohibition",
        "Public games",
        "Refugees",
        "Rioting",
      ],
      "6": [
        "Roundup",
        "Scandal",
        "Serial killer",
        "Shortage",
        "Tournament",
        "Trial",
      ],
    },
  },
  [TABLE_NAMES.CITY_DISTRICT_THEMES]: {
    label: "City: District Themes",
    table: {
      "1": [
        "Catacombs",
        "Civilized NPCs",
        "Construction",
        "Crafts",
        "Criminality",
        "Culture",
      ],
      "2": [
        "Dining",
        "Education",
        "Entertainment",
        "Finance",
        "Foreigners",
        "Ghettoes",
      ],
      "3": [
        "Government",
        "Graveyard",
        "Green space",
        "Industrialization",
        "Judgement",
        "Livestock",
      ],
      "4": [
        "Low class Building",
        "Marketplace",
        "Memorials",
        "Military",
        "Opulence",
        "Pollution",
      ],
      "5": ["Poverty", "Punishment", "Religion", "Science", "Trade", "Trash"],
      "6": [
        "Underworld NPCs",
        "Upper class Building",
        "Vices",
        "Wild NPCs",
        "Wizardry",
        "Wonders",
      ],
    },
  },
  [TABLE_NAMES.CITY_UPPER_CLASS_BUILDINGS]: {
    label: "City: Upper Class Buildings",
    table: {
      "1": [
        "Academy",
        "Alchemist",
        "Archive",
        "Art dealer",
        "Barber",
        "Bookbinder",
      ],
      "2": [
        "Bookseller",
        "Castle",
        "Clockmaker",
        "Clothier",
        "Courthouse",
        "Furrier",
      ],
      "3": [
        "Gallery",
        "Garden",
        "Haberdashery",
        "Jeweler",
        "Law office",
        "Locksmith",
      ],
      "4": ["Lounge", "Manor", "Museum", "Observatory", "Opera house", "Park"],
      "5": [
        "Physician",
        "Printer",
        "Public baths",
        "Restaurant",
        "Salon",
        "Stables",
      ],
      "6": [
        "Taxidermist",
        "Temple",
        "Tobacconist",
        "Townhouse",
        "Winery",
        "Zoo",
      ],
    },
  },
  [TABLE_NAMES.CITY_LOWER_CLASS_BUILDINGS]: {
    label: "City: Lower Class Buildings",
    table: {
      "1": [
        "Apothecary",
        "Asylum",
        "Baker",
        "Brewery",
        "Butcher",
        "Candlemaker",
      ],
      "2": [
        "Catacombs",
        "Cheesemaker",
        "Criminal den",
        "Curiosity shop",
        "Dock",
        "Fighting pit",
      ],
      "3": [
        "Forge",
        "Fortuneteller",
        "Gambling hall",
        "Leatherworks",
        "Marketplace",
        "Mason",
      ],
      "4": [
        "Mill",
        "Moneylender",
        "Orphanage",
        "Outfitter",
        "Prison",
        "Sewers",
      ],
      "5": [
        "Shipyards",
        "Shrine",
        "Stockyard",
        "Stonecarver",
        "Tattooist",
        "Tavern",
      ],
      "6": [
        "Theater",
        "Veterinarian",
        "Warehouse",
        "Watchtower",
        "Weaver",
        "Workshop",
      ],
    },
  },
  [TABLE_NAMES.CITY_ACTIVITIES]: {
    label: "City: Activities",
    table: {
      "1": ["Abduct", "Beg", "Brawl", "Burgle", "Celebrate", "Chase"],
      "2": [
        "Construct",
        "Cook",
        "Dance",
        "Duel",
        "Dungeon Activity",
        "Execute",
      ],
      "3": ["Extinguish", "Extort", "Follow", "Gamble", "Haul", "Interrogate"],
      "4": ["Marry", "Mission", "Mourn", "Party", "Patrol", "Perform"],
      "5": ["Play", "Preach", "Process", "Proclaim", "Protest", "Release"],
      "6": ["Repair", "Riot", "Rob", "Search", "Sell", "Wilderness Activity"],
    },
  },
  [TABLE_NAMES.CITY_BUILDING_ROOMS]: {
    label: "City: Building Room",
    table: {
      "1": ["Arboretum", "Atrium", "Attic", "Aviary", "Ballroom", "Baths"],
      "2": [
        "Bed chamber",
        "Cabinet",
        "Chapel",
        "Cloakroom",
        "Dining room",
        "Dressing room",
      ],
      "3": [
        "Dungeon Room",
        "Garden",
        "Garret",
        "Greenhouse",
        "Junk room",
        "Kitchen",
      ],
      "4": ["Larder", "Library", "Map room", "Menagerie", "Mews", "Nursery"],
      "5": ["Pantry", "Parlor", "Privy", "Root cellar", "Saucery", "Scullery"],
      "6": [
        "Smoking room",
        "Spicery",
        "Still room",
        "Study",
        "Trophy room",
        "Wardrobe",
      ],
    },
  },
  [TABLE_NAMES.CITY_STREET_FEATURES]: {
    label: "City: Street Features",
    table: {
      "1": ["Arcade", "Awnings", "Balconies", "Barricades", "Bridge", "Canal"],
      "2": [
        "Carriages",
        "Catwalks",
        "City Activity",
        "Climbable walls",
        "Clotheslines",
        "Crowd",
      ],
      "3": [
        "Dead end",
        "Dense fog",
        "Downpour",
        "Dungeon activity",
        "Flooding",
        "Food stalls",
      ],
      "4": ["Fountain", "Gates", "Ladders", "Livestock", "Muddy", "Overgrown"],
      "5": [
        "Roof access",
        "Roof gardens",
        "Sewer access",
        "Sinkhole",
        "Slick",
        "Steep roofs",
      ],
      "6": [
        "Steep streets",
        "Steps",
        "Torn up street",
        "Vermin swarms",
        "Well",
        "Wild activity",
      ],
    },
  },
  [TABLE_NAMES.CITY_BUILDING_FEATURE]: {
    label: "City: Building Features",
    table: {
      "1": [
        "Animal nests",
        "Balconies",
        "Basement access",
        "Brightly lit",
        "Broken furniture",
        "Broken glass",
      ],
      "2": [
        "Cabinets",
        "Carpeted floors",
        "Chandeliers",
        "Crawlspaces",
        "Drain pipes",
        "Dumbwaiters",
      ],
      "3": [
        "Echoing marble",
        "Hnaging chains",
        "Huge fireplace",
        "Narrow ledges",
        "Open windows",
        "Ornate weapons",
      ],
      "4": [
        "Overgrown",
        "Patrols",
        "Piles of trash",
        "Pillars",
        "Rotting ceiling",
        "Rotting floors",
      ],
      "5": [
        "Rotting walls",
        "Screens",
        "Servant passages",
        "Sewer access",
        "Shadowy alcoves",
        "Skylights",
      ],
      "6": [
        "Spyholes",
        "Staircases",
        "Tall bookshelves",
        "Unlit",
        "Watchdogs",
        "Window drapes",
      ],
    },
  },
  [TABLE_NAMES.CITY_FACTIONS]: {
    label: "City: Factions",
    table: {
      "1": [
        "Art movement",
        "Beggar's guild",
        "Black market",
        "Brotherhood",
        "City guard",
        "Conspiracy",
      ],
      "2": [
        "Craft guild",
        "Crime family",
        "Crime ring",
        "Dark cult",
        "Explorer's club",
        "Free company",
      ],
      "3": [
        "Gourmand club",
        "Heist crew",
        "Heretical sect",
        "High souncil",
        "Hired killers",
        "Local militia",
      ],
      "4": [
        "National church",
        "Noble house",
        "Outlander clan",
        "Outlaw gang",
        "Political party",
        "Religious order",
      ],
      "5": [
        "Religious sect",
        "Resistance",
        "Royal army",
        "Royal house",
        "Scholar's circle",
        "Secret society",
      ],
      "6": [
        "Spy network",
        "Street artists",
        "Street gang",
        "Street musicians",
        "Theater troupe",
        "Trade company",
      ],
    },
  },
  [TABLE_NAMES.CITY_FACTION_TRAITS]: {
    label: "City: Faction Traits",
    table: {
      "1": [
        "Bankrupt",
        "Bureaucratic",
        "Charitable",
        "Confused",
        "Connected",
        "Corrupt",
      ],
      "2": [
        "Decadent",
        "Decaying",
        "Delusional",
        "Divided",
        "Dwindling",
        "Efficient",
      ],
      "3": [
        "Esoteric",
        "Expanding",
        "Hunted",
        "Incompetent",
        "Incorruptible",
        "Insane",
      ],
      "4": [
        "Insular",
        "Manipulative",
        "Martial",
        "Personality",
        "Pious",
        "Popular",
      ],
      "5": [
        "Righteous",
        "Ruthless",
        "Secret",
        "Subversive",
        "Suppressed",
        "Threatened",
      ],
      "6": [
        "Thriving",
        "Unpopular",
        "Up-and-coming",
        "Wealthy",
        "Well-prepared",
        "Xenophobic",
      ],
    },
  },
  [TABLE_NAMES.CITY_FACTION_GOALS]: {
    label: "City: Faction Goals",
    table: {
      "1": [
        "Advise leader",
        "Avoid detection",
        "Awaken being",
        "Collect artifacts",
        "Construct base",
        "Control faction",
      ],
      "2": [
        "Control politics",
        "Create artifact",
        "Create monster",
        "Defeat faction",
        "Defend border",
        "Defend leader",
      ],
      "3": [
        "Destroy artifacts",
        "Destroy being",
        "Destroy villain",
        "Enforce law",
        "Enrich members",
        "Entertain",
      ],
      "4": [
        "Exchange goods",
        "Hear rumors",
        "Indulge tastes",
        "Infiltrate faction",
        "Map the wild",
        "Overthrow order",
      ],
      "5": [
        "Preserve lineage",
        "Preserve lore",
        "Produce goods",
        "Promote arts",
        "Promote craft",
        "Purge traitors",
      ],
      "6": [
        "Sell services",
        "Share knowledge",
        "Spread beliefs",
        "Summon evil",
        "Survive",
        "Transport goods",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_ENTRANCES]: {
    label: "Dungeon: Entrances",
    table: {
      "1": [
        "All libraries",
        "Beaver dam",
        "Behind waterfall",
        "Chalk rectangle",
        "Chest bottom",
        "Chimney",
      ],
      "2": [
        "Cupboard",
        "Dolmen shadow",
        "Down a well",
        "Fiery pit",
        "Fog road",
        "Forest spring",
      ],
      "3": [
        "Giant book",
        "Gypsy wagon",
        "Hollow tree",
        "Huge keyhole",
        "Iron maiden",
        "Living tattoo",
      ],
      "4": [
        "Magic painting",
        "Man-shape hole",
        "Maze potion",
        "Mirror",
        "Monster mouth",
        "Monster wound",
      ],
      "5": [
        "Narrow alley",
        "Rain door",
        "Sewer grate",
        "Sudden rift",
        "Tidal cave",
        "Tower top",
      ],
      "6": [
        "Tree roots",
        "Under the bed",
        "Unfolded map",
        "Up a tree",
        "Whirlpool",
        "Wine barrel",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_FORMS]: {
    label: "Dungeon: Forms",
    table: {
      "1": ["Arena", "Asylym", "Bank", "Baths", "Body", "Building room"],
      "2": [
        "Casino",
        "Catacombs",
        "Cave",
        "Court",
        "Dungeon Room",
        "Building Room",
      ],
      "3": [
        "Forge",
        "Garden",
        "Hideout",
        "Hotel",
        "Lower class building",
        "Laboratory",
      ],
      "4": ["Library", "Market", "Mine", "Monastery", "Museum", "Nursery"],
      "5": ["Orphanage", "Palace", "Prison", "Sewer", "Ship", "Slave pit"],
      "6": [
        "Temple",
        "Theater",
        "Upper class building",
        "University",
        "Vault",
        "Zoo",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_LAYOUT]: {
    label: "Dungeon: Layout",
    table: {
      "1": [
        "Ant colony",
        "Central hub",
        "Claustrophobic",
        "Crisscrossing",
        "Curved",
        "Disorienting",
      ],
      "2": [
        "Galleria",
        "Geometric",
        "Gonzo",
        "Haphazard",
        "Highly regular",
        "Honeycomb",
      ],
      "3": [
        "Intertwined",
        "Isolated Wings",
        "Layered",
        "Linear",
        "Loops",
        "Many corridors",
      ],
      "4": [
        "Mazes",
        "Mix of layouts",
        "Multiple hubs",
        "No corridors",
        "Open plan",
        "Open voids",
      ],
      "5": [
        "Organic",
        "Oversized",
        "Recursive",
        "Repetitive",
        "Sprawling",
        "Suspended",
      ],
      "6": [
        "Symbol shape",
        "Tall and narrow",
        "Themed zones",
        "Vertical",
        "Winding",
        "Ziggurat",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_RUINATIONS]: {
    label: "Dungeon: Ruinations",
    table: {
      "1": [
        "Arcane disaster",
        "Army invasion",
        "Cannibalism",
        "Civil war",
        "Collapse",
        "Crystal growth",
      ],
      "2": [
        "Curse",
        "Degeneration",
        "Earthquake",
        "Eruption",
        "Evil unearthed",
        "Experiments",
      ],
      "3": ["Explosion", "Famine", "Fire", "Flooding", "Fungus", "Haunting"],
      "4": [
        "Ice",
        "Insanity",
        "Lava flow",
        "Magical sleep",
        "Melted",
        "Monster attack",
      ],
      "5": [
        "Mutation",
        "Outsider attack",
        "Overgrowth",
        "Petrification",
        "Plague",
        "Planar overlay",
      ],
      "6": [
        "Poison gas",
        "Resources gone",
        "Revolt",
        "Risen dead",
        "Too many traps",
        "War",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_REWARDS]: {
    label: "Dungeon: Rewards",
    table: {
      "1": [
        "Ancient lore",
        "Animal ally",
        "Army",
        "Blessing",
        "Blueprints",
        "Cultural artifact",
      ],
      "2": [
        "Enemy weakness",
        "Faction ally",
        "Forewarning",
        "Guide",
        "Holy relic",
        "Influential ally",
      ],
      "3": [
        "Instructions",
        "Jewels",
        "Key",
        "Lost formula",
        "Machine",
        "Magic item",
      ],
      "4": [
        "Magical ally",
        "Map",
        "Martial ally",
        "Masterpiece",
        "Monster ally",
        "Oracle",
      ],
      "5": [
        "Piles of loot",
        "Planar portal",
        "Prophecy",
        "Renown",
        "Spell",
        "Transformation",
      ],
      "6": [
        "Transport",
        "Treasure item",
        "Uncovered plot",
        "Valuable Material",
        "Vision",
        "Weapon",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_ACTIVITIES]: {
    label: "Dungeon: Activities",
    table: {
      "1": [
        "Besiege",
        "Capture",
        "City activity",
        "Collect",
        "Construct",
        "Control",
      ],
      "2": ["Deliver", "Demolish", "Escape", "Feed", "Fortify", "Guard"],
      "3": ["Hide", "Hunt", "Loot", "Map", "Mine", "Monster tactic"],
      "4": [
        "Negotiate",
        "Patrol",
        "Perform ritual",
        "Purge",
        "Question",
        "Raid",
      ],
      "5": ["Repair", "Rescue", "Research", "Revive", "Riddle", "Scavenge"],
      "6": [
        "Seize",
        "Tunnel",
        "Unearth",
        "Vandalize",
        "Wild activity",
        "Worship",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_ROOMS]: {
    label: "Dungeon: Rooms",
    defaultNumRolls: 5,
    table: {
      "1": [
        "Armory",
        "Banquet hall",
        "Barracks",
        "Building room",
        "Catacombs",
        "Cavery",
      ],
      "2": [
        "Chasm",
        "Courtyard",
        "Crypt",
        "Dormitory",
        "Fighting pit",
        "Forge",
      ],
      "3": [
        "Fountain",
        "Gate house",
        "Guard room",
        "Kennel",
        "Lower class building",
        "Laboratory",
      ],
      "4": ["Mess hall", "Mine shaft", "Museum", "Oubliette", "Pool", "Prison"],
      "5": [
        "Record room",
        "Shrine",
        "Slaugterhouse",
        "Stables",
        "Storeroom",
        "Throne room",
      ],
      "6": [
        "Torture room",
        "Treasury",
        "Upper class building",
        "Valut",
        "Well",
        "Workshop",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_ROOM_DETAILS]: {
    label: "Dungeon: Room Details",
    table: {
      "1": [
        "Bas-relief",
        "Blood trail",
        "Bones",
        "Chains",
        "Chalk marks",
        "Claw marks",
      ],
      "2": [
        "Corpses",
        "Cracked beams",
        "Crumbling walls",
        "Decaying food",
        "Decaying nest",
        "Dripping water",
      ],
      "3": [
        "Fading murals",
        "Faint breeze",
        "Faint footsteps",
        "Fallen pillars",
        "Fungus",
        "Furninture",
      ],
      "4": [
        "Graffiti",
        "Mosaics",
        "Recent repairs",
        "Rotting books",
        "Rubble",
        "Shed skin",
      ],
      "5": [
        "Slime trails",
        "Spider webs",
        "Stalactities",
        "Stench",
        "Smoke stains",
        "Thick dusk",
      ],
      "6": [
        "Torn clothes",
        "Tree roots",
        "Unusual smell",
        "Vibrations",
        "Vines",
        "Whispers",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_TRICKS]: {
    label: "Dungeon: Tricks",
    table: {
      "1": [
        "Absorption",
        "Activation",
        "Animation",
        "Blessings",
        "Communication",
        "Confusion",
      ],
      "2": [
        "Consumption",
        "Creation",
        "Curses",
        "Deception",
        "Duplication",
        "Ethereal effect",
      ],
      "3": [
        "Exchange",
        "Imprisonment",
        "Instructions",
        "Interrogation",
        "Mind-control",
        "Mission",
      ],
      "4": [
        "Mood-alteration",
        "Nullification",
        "Physical effect",
        "Planeshift",
        "Protection",
        "Rejuvenation",
      ],
      "5": [
        "Release",
        "Reversal",
        "Rotation",
        "Scrying",
        "Size-alteration",
        "Summoning",
      ],
      "6": [
        "Theft",
        "Time-alteration",
        "Transformation",
        "Transmutation",
        "Transportation",
        "Wonder",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_HAZARDS]: {
    label: "Dungeon: Hazards",
    table: {
      "1": [
        "Acid drip",
        "Bloodsuckers",
        "Cave-in",
        "Choking dust",
        "Crude oil",
        "Crystal shards",
      ],
      "2": [
        "Deafening noise",
        "Dense fog",
        "Ensnaring vines",
        "Fallen floor",
        "Flooding",
        "Freezing",
      ],
      "3": [
        "Geysers",
        "Magma",
        "Magnetic field",
        "Mud flow",
        "Narrow ledge",
        "Narrow passage",
      ],
      "4": [
        "Poison goo",
        "Poison plants",
        "Precipice",
        "Quicksand",
        "Radiation",
        "Rockslide",
      ],
      "5": [
        "Rotten ceiling",
        "Rotten floor",
        "Sinkhole",
        "Slippery slope",
        "Spider webs",
        "Spores",
      ],
      "6": [
        "Steam vents",
        "Strong winds",
        "Tar pit",
        "Tight passage",
        "Toppling object",
        "Toxic fumes",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_TRAP_EFFECTS]: {
    label: "Dungeon: Trap Effects",
    table: {
      "1": [
        "Acid pool",
        "Adhesive",
        "Alarm",
        "Armor melts",
        "Bear trap",
        "Blinding spray",
      ],
      "2": [
        "Blunt pendulum",
        "Boiling tar",
        "Collapsing floor",
        "Crocodile pit",
        "Crushing walls",
        "Deep pit",
      ],
      "3": [
        "Falling cage",
        "Falling ceiling",
        "Fills with sand",
        "Flooding",
        "Giant magnet",
        "Hard vacuum",
      ],
      "4": [
        "Lava flow",
        "Lightning",
        "Living statues",
        "Missile fire",
        "Monster freed",
        "Net trap",
      ],
      "5": [
        "Pendulum blade",
        "Poison gas",
        "Poison needle",
        "Quicksand",
        "Rage gas",
        "Rolling boulder",
      ],
      "6": [
        "Room freezes",
        "Room on fire",
        "Sleeping gas",
        "Spiked pit",
        "Tombs open",
        "Wall spikes",
      ],
    },
  },
  [TABLE_NAMES.DUNGEON_TRAP_TRIGGERS]: {
    label: "Dungeon: Trap Triggers",
    table: {
      "1": ["Blow", "Break", "Burn", "Choice", "Countdown", "Darkness"],
      "2": ["Drain", "Eat", "Insert", "Kill", "Knock", "Light"],
      "3": ["Magic", "Melody", "Noise", "Open", "Phrase", "Pour"],
      "4": ["Press", "Proximity", "Pull", "Read", "Reflect", "Release"],
      "5": ["Remove", "Retrieve", "Rudeness", "Shut", "Sit", "Sleep"],
      "6": ["Slide", "Touch", "Turn", "Unbalance", "Unearth", "Write"],
    },
  },
  [TABLE_NAMES.ITEM_MISCELLANEOUS]: {
    label: "Item: Miscellaneous",
    table: {
      "1": [
        "Bowl",
        "Brass bell",
        "Brooch",
        "Carved figuring",
        "Cup",
        "Deck of cards",
      ],
      "2": [
        "Drawing",
        "Foreign coin",
        "Game piece",
        "Glass eye",
        "Glass jar",
        "Hair comb",
      ],
      "3": [
        "Handkerchief",
        "Hinged box",
        "Hourglass",
        "Human tooth",
        "Hunting horn",
        "Loaded dice",
      ],
      "4": [
        "Long fork",
        "Numbered key",
        "Oil lamp",
        "Old doll",
        "Paint pot",
        "Pencil",
      ],
      "5": [
        "Purse",
        "Quill pen",
        "Salve",
        "Scissors",
        "Scroll",
        "Sealed letter",
      ],
      "6": [
        "Sewing needle",
        "Shaving razor",
        "Silver button",
        "Skull",
        "Tobacco pipe",
        "Wine bottle",
      ],
    },
  },
  [TABLE_NAMES.ITEM_WORN]: {
    label: "Item: Worn",
    table: {
      "1": ["Belt", "Blouse", "Boots", "Bracelet", "Breastplate", "Brigandine"],
      "2": ["Cincture", "Cloak", "Coat", "Dress", "Earing", "Eyepatch"],
      "3": ["Gauntlets", "Glove", "Gown", "Hat", "Helmet", "Hose"],
      "4": [
        "Leather armor",
        "Locket",
        "Mail shirt",
        "Mask",
        "Necklace",
        "Padded armor",
      ],
      "5": ["Plate mail", "Ring", "Robe", "Sandals", "Scarf", "Shirt"],
      "6": ["Shoes", "Skirt", "Slippers", "Socks", "Trousers", "Veil"],
    },
  },
  [TABLE_NAMES.ITEM_WEAPON]: {
    label: "Item: Weapon",
    table: {
      "1": [
        "Arming sword",
        "Backsword",
        "Battleaxe",
        "Blowpipe",
        "Claymore",
        "Club",
      ],
      "2": [
        "Crossbow",
        "Cutlass",
        "Dagger",
        "Flail",
        "Fllanged mace",
        "Glaive",
      ],
      "3": [
        "Halberd",
        "Hammer",
        "Hatchet",
        "Horsebow",
        "Hunting knife",
        "Lance",
      ],
      "4": ["Longbow", "Longsword", "Mace", "Maul", "Morningstar", "Pike"],
      "5": ["Scimitar", "Shortbow", "Sickle", "Sling", "Spear", "Staff"],
      "6": [
        "Stake",
        "Stiletto",
        "Throwing axe",
        "Warhammer",
        "Warpick",
        "Whip",
      ],
    },
  },
  [TABLE_NAMES.ITEM_BOOK_SUBJECT]: {
    label: "Item: Book Subjects",
    table: {
      "1": [
        "Alchemy",
        "Art",
        "Astrology",
        "Blackmail",
        "Charts & maps",
        "Conspiracies",
      ],
      "2": [
        "Cookbook",
        "Criminals",
        "Divination",
        "Etiquette",
        "Fashion",
        "Genealogy",
      ],
      "3": ["Hagiography", "History", "Journal", "Language", "Laws", "Letters"],
      "4": [
        "Lost empires",
        "Lost places",
        "Love poems",
        "Monsters",
        "Mythology",
        "Old customs",
      ],
      "5": [
        "Oratory",
        "Propaganda",
        "Prophecies",
        "Siegecraft",
        "Songs",
        "State secrets",
      ],
      "6": [
        "Sword fighting",
        "Theology",
        "Treasuers",
        "War chronicle",
        "Who's who",
        "Witch hunting",
      ],
    },
  },
  [TABLE_NAMES.ITEM_TOOL]: {
    label: "Item: Tool",
    table: {
      "1": [
        "Acid flask",
        "Bear trap",
        "Bellows",
        "Bolt-cutters",
        "Chain",
        "Chisel",
      ],
      "2": [
        "Crowbar",
        "Door ram",
        "Ear trumpet",
        "Fire oil",
        "Fishing hook",
        "Goggles",
      ],
      "3": [
        "Grappling hook",
        "Grease",
        "Hacksaw",
        "Hammer",
        "Hnad drill",
        "Lantern",
      ],
      "4": [
        "Lens",
        "Lock/key",
        "Lockpicks",
        "Manacles",
        "Metal file",
        "Mortar/pestle",
      ],
      "5": ["Needle", "Pickaxe", "Pitchfork", "Pliers", "Pole", "Pulleys"],
      "6": ["Rope", "Scissors", "Shovel", "Spikes", "Steel wire", "Tongs"],
    },
  },
  [TABLE_NAMES.ITEM_POTIONS]: {
    label: "Item: Potions",
    table: {
      "1": [
        "Animal form",
        "Body swap",
        "Camouflage",
        "Control animals",
        "Control element",
        "Cure affliction",
      ],
      "2": [
        "Detect evil",
        "Detect gold",
        "Detect hidden",
        "Direction sense",
        "Element form",
        "Element skin",
      ],
      "3": [
        "Extra arm",
        "Flight",
        "Ghost-speech",
        "Heat vision",
        "Insanity",
        "Invulnerable",
      ],
      "4": [
        "Item-form",
        "Magic immunity",
        "Mirror image",
        "Monster ability",
        "Monster feature",
        "Monster trait",
      ],
      "5": [
        "Mutation",
        "Night vision",
        "Random spell",
        "Restore health",
        "Speed",
        "Stretchy",
      ],
      "6": [
        "Super-jump",
        "Super-strength",
        "Telekinesis",
        "Tongues",
        "Water-breathing",
        "Water-walking",
      ],
    },
  },
  [TABLE_NAMES.ITEM_MAGICAL_INGREDIENT]: {
    label: "Item: Magical Ingredients",
    table: {
      "1": [
        "Ancient liquor",
        "Animal",
        "Blind eye",
        "Boiled cat",
        "Book page",
        "Bottled fog",
      ],
      "2": [
        "Coffin nail",
        "Corpse's hair",
        "Crossroad dust",
        "Cultist entrails",
        "Edible plant",
        "Exotic spice",
      ],
      "3": [
        "Killer's hand",
        "King's tooth",
        "Last breath",
        "Liar's tongue",
        "Lightning bolt",
        "Lodestone",
      ],
      "4": [
        "Monk's vow",
        "Monster feature",
        "Newborn's cry",
        "Oil portrait",
        "Physical element",
        "Poisonous plant",
      ],
      "5": [
        "Potion",
        "Pyre ember",
        "Queen's bee",
        "Queen's blood",
        "Ship's barnacle",
        "Star-metal",
      ],
      "6": [
        "Thief's finger",
        "Tomb flower",
        "Valuable material",
        "Wedding ring",
        "Widow's tear",
        "Wizard skull",
      ],
    },
  },
  [TABLE_NAMES.ITEM_TREASURE_ITEM]: {
    label: "Item: Treasure Item",
    table: {
      "1": [
        "Alchemy recipe",
        "Amulet",
        "Astrolabe",
        "Blueprints",
        "Calligraphy",
        "Carpet",
      ],
      "2": ["Compas", "Contract", "Crown", "Crystal", "Deed", "Embroidery"],
      "3": [
        "Fine china",
        "Fine liquor",
        "Instrument",
        "Magical book",
        "Microscope",
        "Music box",
      ],
      "4": [
        "Orrery",
        "Painting",
        "Perfume",
        "Prayer book",
        "Printing block",
        "Rare textile",
      ],
      "5": [
        "Royal robes",
        "Saint's relic",
        "Scrimshaw",
        "Sextant",
        "Sheet music",
        "Signet ring",
      ],
      "6": [
        "Silverware",
        "Spices",
        "Spyglass",
        "Tapestry",
        "Telescope",
        "Treasure map",
      ],
    },
  },
  [TABLE_NAMES.ITEM_TREASURE_TRAIT]: {
    label: "Item: Treasurer trait",
    table: {
      "1": ["Altered", "Ancient", "Blessed", "Bulky", "Compact", "Consumable"],
      "2": [
        "Cultural value",
        "Cursed",
        "Damaged",
        "Disguised",
        "Draws enemies",
        "Effect",
      ],
      "3": [
        "Element",
        "Embellished",
        "Encoded",
        "Exotic",
        "Extra-planar",
        "Famous",
      ],
      "4": [
        "Forbidden",
        "Fragile",
        "Heavy",
        "Immovable",
        "Impracticable",
        "Indestructible",
      ],
      "5": [
        "Intelligent",
        "Masterwork",
        "Military value",
        "Non-human",
        "Owned",
        "Partial",
      ],
      "6": [
        "Political value",
        "Religious value",
        "Repiared",
        "Royal",
        "Toxic",
        "Vile",
      ],
    },
  },
  [TABLE_NAMES.ITEM_VALUABLE_MATERIAL]: {
    label: "Item: Valuable Material",
    table: {
      "1": [
        "Alabaster",
        "Amber",
        "Aquamarine",
        "Azurite",
        "Beryl",
        "Black pearl",
      ],
      "2": [
        "Bloodstone",
        "Bone china",
        "Chalcedony",
        "Cinnabar",
        "Coral",
        "Diamond",
      ],
      "3": ["Ebony", "Emerald", "Fire agate", "Garnet", "Gold", "Ivory"],
      "4": ["Jade", "Jasper", "Jet", "Lapis lazuli", "Malachite", "Moonstone"],
      "5": ["Onyx", "Opal", "Pearl", "Platinum", "Porcelain", "Ruby"],
      "6": [
        "Sapphire",
        "Serpentine",
        "Silver",
        "Star iron",
        "Topaz",
        "Turquoise",
      ],
    },
  },

  [TABLE_NAMES.WILD_REGION]: {
    label: "Wilderness: Regions",
    table: {
      "1": ["Ashy", "Badlands", "Bay", "Beach", "Delta", "Desert"],
      "2": [
        "Dry lands",
        "Dune sea",
        "Dust bowl",
        "Fjords",
        "Flood lands",
        "Foothills",
      ],
      "3": ["Forest", "Glaciers", "Heath", "Highlands", "Hills", "Ice fields"],
      "4": [
        "Jungle",
        "Lowlands",
        "Mesas",
        "Moor",
        "Mountains",
        "Petrified forest",
      ],
      "5": [
        "Plains",
        "Rainforest",
        "Riverlands",
        "Salt pan",
        "Savanna",
        "Steppe",
      ],
      "6": [
        "Taiga",
        "Thickets",
        "Tundra",
        "Volcanic plain",
        "Wetlands",
        "Woodlands",
      ],
    },
  },
  [TABLE_NAMES.WILD_REGION_TRAITS]: {
    label: "Wilderness: Region Traits",
    table: {
      "1": ["Ashen", "Blasted", "Blighted", "Broken", "Consuming", "Corrupted"],
      "2": [
        "Creeping",
        "Desolate",
        "Dungeon ruination",
        "Eternal",
        "Ethereal effect",
        "Forsaken",
      ],
      "3": ["Frozen", "Haunted", "Howling", "Jagged", "Lonely", "Misty"],
      "4": [
        "Perilous",
        "Petrified",
        "Phantasmal",
        "Ravenous",
        "Savage",
        "Shadowy",
      ],
      "5": [
        "Shifting",
        "Shivering",
        "Sinister",
        "Sinking",
        "Smoldering",
        "Sweltering",
      ],
      "6": [
        "Thorny",
        "Thundering",
        "Torrential",
        "Physical effect",
        "Wandering",
        "Withered",
      ],
    },
  },
  [TABLE_NAMES.WILD_LANDMARKS]: {
    label: "Wilderness: Landmarks",
    table: {
      "1": ["Bog", "Boulder field", "Butte", "Cave", "Cliff", "Crag"],
      "2": ["Crater", "Creek", "Crossing", "Ditch", "Field", "Forest"],
      "3": ["Grove", "Hill", "Hollow", "Hot springs", "Lair", "Lake"],
      "4": ["Lakebed", "Marsh", "Mesa", "Moor", "Pass", "Pit"],
      "5": ["Pond", "Rapids", "Ravine", "Ridge", "Rise", "River"],
      "6": ["Rockslide", "Spring", "Swamp", "Thickets", "Valley", "Waterfall"],
    },
  },
  [TABLE_NAMES.WILD_STRUCTURES]: {
    label: "Wilderness: Structures",
    table: {
      "1": [
        "Altar",
        "Aqueduct",
        "Bandit's camp",
        "Battlefield",
        "Bonfire",
        "Bridge",
      ],
      "2": ["Cairn", "Crossroads", "Crypt", "Dam", "Dungeon", "Farm"],
      "3": [
        "Ford",
        "Fortress",
        "Gallows",
        "Graveyard",
        "Hedge",
        "Hunter's camp",
      ],
      "4": ["Inn", "Lumber camp", "Mine", "Monastery", "Monument", "Orchard"],
      "5": ["Outpost", "Pasture", "Ruin", "Seclusion", "Shack", "Shrine"],
      "6": [
        "Standing stone",
        "Temple",
        "Village",
        "Wall",
        "Watchtower",
        "Waystone",
      ],
    },
  },
  [TABLE_NAMES.WILD_DISCOVERIES]: {
    label: "Wilderness: Discoveries",
    table: {
      "1": [
        "Blood stains",
        "Bones",
        "Broken weapons",
        "Burrow",
        "City activity",
        "Civilized NPC",
      ],
      "2": [
        "Cut ropes",
        "Dead animal",
        "Dungeon activity",
        "Food scraps",
        "Grave marker",
        "Human corpse",
      ],
      "3": [
        "Item",
        "Lost NPC",
        "Magical effect",
        "Map",
        "Message",
        "Migration",
      ],
      "4": ["Mutation", "Nest", "Portal", "Resources", "Rift", "Strange plant"],
      "5": [
        "Stunned NPC",
        "Supplies",
        "Torn flag",
        "Tracks",
        "Trap",
        "Treasure cache",
      ],
      "6": [
        "Underground NPC",
        "Wild activity",
        "Wild landmark",
        "Wild structure",
        "Wild NPC",
        "Wizard fight",
      ],
    },
  },
  [TABLE_NAMES.WILD_ACTIVITIES]: {
    label: "Wilderness: Activities",
    table: {
      "1": ["Ambush", "Argue", "Birth", "Build", "Bury", "Capture"],
      "2": [
        "City activity",
        "Convene",
        "Demolish",
        "Die",
        "Duel",
        "Dungeon activity",
      ],
      "3": ["Eat", "Excavate", "Feast", "Felling", "Fish", "Flee"],
      "4": ["Forage", "Hunt", "Mark", "Raid", "Rescue", "Rest"],
      "5": ["Sacrifice", "Scout", "Sing", "Skin", "Skirmish", "Slay"],
      "6": ["Sleep", "Swim", "Track", "Trap", "Wander", "Worship"],
    },
  },
  [TABLE_NAMES.WILD_HAZARDS]: {
    label: "Wilderness: Hazards",
    table: {
      "1": [
        "Avalanche",
        "Blizzard",
        "Brushfire",
        "Cloudburst",
        "Cyclone",
        "Dense fog",
      ],
      "2": [
        "Downpour",
        "Drizzle",
        "Dust storm",
        "Earthquake",
        "Eruption",
        "Flooding",
      ],
      "3": [
        "Forest fire",
        "Hail",
        "Heat wave",
        "Hurricane",
        "Ice storm",
        "Light mist",
      ],
      "4": [
        "Locust swarm",
        "Magma flow",
        "Meteor strike",
        "Monsoon",
        "Mudflow",
        "Mudslide",
      ],
      "5": [
        "Predator",
        "Quicksand",
        "Rain of frogs",
        "Rockslide",
        "Sandstorm",
        "Sleet",
      ],
      "6": [
        "Snow",
        "Stampede",
        "Thunderstorm",
        "Tsunami",
        "Whirlpool",
        "Windstorm",
      ],
    },
  },
  [TABLE_NAMES.WILD_EDIBLE_PLANTS]: {
    label: "Wilderness: Edible Plants",
    table: {
      "1": [
        "Acorns",
        "Apples",
        "Asparagus",
        "Blackberries",
        "Blueberries",
        "Carrots",
      ],
      "2": [
        "Cattail",
        "Cherries",
        "Chickweed",
        "Chicory",
        "Clover",
        "Dandelion",
      ],
      "3": [
        "Dead-nettle",
        "Elderberries",
        "Fireweed",
        "Gooseberries",
        "Hazelnuts",
        "Henbit",
      ],
      "4": [
        "Hickory nuts",
        "Honeysuckle",
        "Leeks",
        "Milk thistle",
        "Mint",
        "Mulberries",
      ],
      "5": [
        "Mushrooms",
        "Mustard",
        "Onion",
        "Pecans",
        "Persimmons",
        "Raspberries",
      ],
      "6": [
        "Strawberries",
        "Walnuts",
        "Watercress",
        "Wild garlic",
        "Wild grapes",
        "Wood sorrel",
      ],
    },
  },
  [TABLE_NAMES.WILD_POISONOUS_PLANTS]: {
    label: "Wilderness: Poisonous Plants",
    table: {
      "1": [
        "Angel's trumpet",
        "Baneberry",
        "Belladona",
        "Black truffle",
        "Bleeding heart",
        "Celandine",
      ],
      "2": [
        "Cocklebur",
        "Columbine",
        "Crowncup",
        "Dath cap",
        "Dumbcane",
        "Foxglove",
      ],
      "3": ["Hemlock", "Hogweed", "Holly", "Horse chestnut", "Hyacinth", "Ivy"],
      "4": [
        "Jessamine",
        "Kudu",
        "Larkspur",
        "Mandrake",
        "Mangrove",
        "Mistletoe",
      ],
      "5": [
        "Moonflower",
        "Nightshade",
        "Oleander",
        "Ragwort",
        "Reindeer lichen",
        "Snakewood",
      ],
      "6": [
        "Spindle",
        "Stinkhorn",
        "Waxcap",
        "Wine-cap",
        "Wolfsbane",
        "Wormwood",
      ],
    },
  },
  [TABLE_NAMES.INN_ADJECTIVES]: {
    label: "Inn: Adjectives",
    table: {
      "1": ["Bellowing", "Blazing", "Bleak", "Blessed", "Bloody", "Crimson"],
      "2": ["Cunning", "Copper", "Dancing", "Dead", "Drunken", "Flying"],
      "3": ["Ghastly", "Golden", "Helpful", "Hideous", "Howling", "Hungry"],
      "4": ["Moldy", "Muttering", "Nimble", "Oozing", "Petrified", "Prancing"],
      "5": ["Romantic", "Salty", "Singing", "Shivering", "Shrieking", "Silver"],
      "6": ["Smoking", "Thirsty", "Tipsy", "Wicked", "Whistling", "Wanton"],
    },
  },
  [TABLE_NAMES.INN_NOUNS]: {
    label: "Inn: Nouns",
    table: {
      "1": ["Axe", "Barrel", "Bear", "Bell", "Boot", "Bowl"],
      "2": ["Bucket", "Candle", "Cock", "Cow", "Dragon", "Egg"],
      "3": ["Elephant", "Flea", "Fork", "Giant", "Griffin", "Hart"],
      "4": ["Hog", "Hound", "Lamb", "Lion", "Mackeral", "Maid"],
      "5": ["Monk", "Moon", "Pipe", "Prince", "Rat", "Skull"],
      "6": ["Spoon", "Star", "Swan", "Sword", "Whale", "Wife"],
    },
  },
  [TABLE_NAMES.INN_QUIRKS]: {
    label: "Inn: Quirks",
    table: {
      "1": [
        "100 years in past",
        "Always night",
        "Animal fights",
        "Bard duels",
        "Bigger inside",
        "Black market",
      ],
      "2": [
        "Brand new",
        "Cannibals",
        "City activity",
        "Constant party",
        "Dancing contest",
        "Dead drop",
      ],
      "3": [
        "Dungeon form",
        "Expensive",
        "Faction hangout",
        "Faction traits",
        "Famous chef",
        "Fey patrons",
      ],
      "4": [
        "Fight club",
        "Five floors",
        "Ghost staff",
        "Haunted",
        "Hidegout",
        "Inn/building",
      ],
      "5": [
        "Magic sword",
        "Magically moves",
        "Mercs for hire",
        "NPC hangout",
        "Preaching",
        "Secure storage",
      ],
      "6": [
        "Staff are kids",
        "Talking painting",
        "Underground",
        "VIP lounge",
        "Voice in well",
        "Women only",
      ],
    },
  },
  [TABLE_NAMES.CURSED]: {
    label: "General: Cursed",
    table: {
      "1": ["Yes"],
      "2": ["No"],
      "3": ["No"],
      "4": ["No"],
      "5": ["No"],
      "6": ["No"],
    },
  },
  [TABLE_NAMES.ALIGNMENT]: {
    label: "General: Alignment",
    table: {
      "1": ["Lawful Good", "Lawful Neutral", "Lawful Evil"],
      "2": ["Neutral Good", "Neutral", "Neutral Evil"],
      "3": ["Chaotic Good", "Chaotic Neutral", "Chaotic Evil"],
    },
  },
};

export const CHARACTER_PROPERTY_TO_FLAVOR_TEXT = {
  Male: "male",
  Female: "female",
  "Hard to tell": "gender fluid",
  Birthmark: "a birthmark",
  "Brand mark": "a brand mark",
  "Broken nose": "a crooked nose",
  "Exotic accent": "an exotic accent",
  "Gold tooth": "a smile with a gold tooth",
  "Hoarse voice": "a hoarse voice",
  "One-eyed": "an eye patch",
  Sunburned: "sunburned skin",
  Topknot: "a topknot",
  Harengon: "anthropomorphic rabbit",
};

export const getTableLabel = (table) => {
  return TABLES[table]?.label || "UNNAMED_TABLE";
};
